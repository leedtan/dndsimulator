...
def get_closest_within_dist(coor, distance, table, char):
    ...
    options = [
        (x, y_extreme) for x in range(min_x, max_x + 1) for y_extreme in [min_y, max_y]
    ] + [
    ...
    options = [
        ...
        if option[0] >= 0
        and option[1] >= 0
        and option[0] < table.shape[0]
        and option[1] < table.shape[1]
    ]
    options = [
        option for option in options if table[option] == 0 or table[option] == char
    ]
    return options
