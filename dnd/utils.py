    ...
    for i in range(-1, 2):
        for j in range(-1, 2):
            if i == j == 0:
                ...
                if table[next_loc] == 0 or next_loc == destination:
                    neighbors.append((coor[0] + i, coor[1] + j))
                    # yield coor[0] + i, coor[1] + j
    return neighbors
...
# def move_to_enemy(graph: WeightedGraph, start: Location, goal: Location):
def move_to_enemy(self, closest_enemy, table):
    frontier = PriorityQueue()
    frontier.put(self.coor, 0)
    # came_from, cost_so_far = defaultdict(lambda: None), defaultdict((lambda: 0))
    # came_from[(self.coor[0], self.coor[1])] = None
    # cost_so_far[(self.coor[0], self.coor[1])] = 0
    ...
    while not frontier.empty():
        ...
        #         "distance": distance,
        #         "current": current,
        #         "closest_enemy.coor": closest_enemy.coor,
        #         "self.coor": self.coor,
        #         "neighbors": neighbors,
        #     }
        # )
        ...
        for next_item in neighbors:
            # new_cost = cost_so_far[current] + calc_distance(
            #     current, next_item
            # )
...
# def move_to_enemy(self, closest_enemy, table):
#     value_estimates = defaultdict(lambda: 9999)
#     pq = PriorityQueue()
#     manhattan_distance = heuristic(self, closest_enemy)
#     moved = 0
